...
class dfs_cycle {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0 ;i<edges;i++)
        {
            ...
            visited_edges[src-1][destination-1] = 1;
        ...
        for(int i = 0;i<vertices;i++)
        {
            for(int j =0;j<vertices;j++)
            {
                if(visited_edges[i][j]!=0)
                {
                    if(i==j)
                        flag = 1;
                    else if((all_vertices.get(i+1)[0]>=all_vertices.get(j+1)[0] && all_vertices.get(i+1)[0]<=all_vertices.get(j+1)[1])&&(all_vertices.get(i+1)[1]>=all_vertices.get(j+1)[0] && all_vertices.get(i+1)[1]<=all_vertices.get(j+1)[1]))
                        flag = 1;
                    else
                        continue;
                }
        ...
        System.out.println("The Graph does not contain the cycle");
        sc.close();
    }
}
